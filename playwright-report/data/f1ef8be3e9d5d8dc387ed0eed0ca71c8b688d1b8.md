...
- Name: Usuário pode ordenar produtos do menor para o maior preço
- Location: C:\Users\amori\OneDrive\Desktop\projeto-qa-automacao\tests\ordenacao\ordenar-produtos.spec.js:4:1
...
Call log:
  - waiting for locator('[data-test="product_sort_container"]')
    ...
    at C:\Users\amori\OneDrive\Desktop\projeto-qa-automacao\tests\ordenacao\ordenar-produtos.spec.js:11:14
```
...
```ts
   ...
   2 | const users = require('../../fixtures/users.json');
   3 |
   4 | test('Usuário pode ordenar produtos do menor para o maior preço', async ({ page }) => {
   5 |   await page.goto('https://www.saucedemo.com/');
   6 |
   7 |   await page.fill('#user-name', users.validUser.username);
   8 |   await page.fill('#password', users.validUser.password);
   9 |   await page.click('#login-button');
  10 |
> 11 |   await page.selectOption('[data-test="product_sort_container"]', 'lohi');
     |              ^ Error: page.selectOption: Test timeout of 30000ms exceeded.
  12 |
  13 |   const firstItem = await page.locator('.inventory_item_price').first();
  14 |   const price = await firstItem.textContent();
  15 |
  16 |   expect(price).toBe('$7.99');
  17 | });
  18 |
```